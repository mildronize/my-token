...
// TestCostForUsage_TableDriven mirrors internal/domain/usage's own
// TestCostForUsage_TableDriven test-for-test (same rates, same cases) —
// contract's Testing Decisions: "pure, table-driven unit tests — no
// framework dependency, ported straight from the reference collector's
// own PRICING table." This package's copy is the collector's own local
// estimate (doc.go), not the authoritative one, but the shape and rates
// must still match ticket 11's server-side copy exactly, since both were
// ported from the same source table.
